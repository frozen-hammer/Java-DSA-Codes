public class SudukoSolver {
    ...
    private boolean solveSudoku2(int row, int col){
         ...
         if(col == MAX_SIZE){
          ...
          // if we cross  the last row +1 (9) , we solve the sudoku (Termination Case)
           if(row == MAX_SIZE){ // Termination Case
            ...
            // if cell i not empty so move to the next Column
            if(board[row][col]!='.'){
                return solveSudoku2(row, col+1);
            }
             ...
             for(int i = 1; i<=MAX_SIZE; i++){
                 ...
                 if(isCorrectToPlaceANumber(row, col, (char) (i+'0'))){
                    ...
                    boolean result = solveSudoku2(row, col+1);
                    if(result){
                        // Solved the Sudoku
